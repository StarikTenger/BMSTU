...
optional<Tokenizer::Token> Tokenizer::read_token() {
	...
	case '[':
		str_it++;
		return Token(TokenType::BracketL, "[", pos() - 1);
	case ']':
		str_it++;
		return Token(TokenType::BracketR, "]", pos() - 1);
	case '|':
		str_it++;
	...
	case '*':
		str_it++;
		return Token(TokenType::Star, "*", pos() - 1);
	default: {
...
ostream& operator<<(ostream& os, const Tokenizer::TokenType& tkn) {
	unordered_map<Tokenizer::TokenType, string> names = {
		...
		{Tokenizer::TokenType::BracketL, "BracketL"},
		{Tokenizer::TokenType::BracketR, "BracketR"},
		{Tokenizer::TokenType::Bar, "Bar"},
		{Tokenizer::TokenType::Dot, "Dot"},
		{Tokenizer::TokenType::Eq, "Eq"},
		{Tokenizer::TokenType::Star, "Star"},
